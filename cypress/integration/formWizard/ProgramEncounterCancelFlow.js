...
describe("Program Encounter Cancel Flow tests for form wizard", () => {
  ...
  it("Cancelling new program encounter should not move to next page in case of validation error", () => {
    dashboardPage.cancelProgramEncounter("Program1", "ProgramEncounter1");
  ...
  it("All the FE in the form are hidden", () => {
    ...
    dashboardPage.cancelProgramEncounter("Program1", "ProgramEncounter1");
    wizardPage.assertIfPageContains("Cancel Date");
    wizardPage.checkScenarioHideAllFEG();
  });
  it("An Empty encounter without any FEG created", () => {
    dashboardPage.cancelProgramEncounter("Program1", "Empty Program Encounter");
    wizardPage.assertIfPageContains("Cancel Date");
