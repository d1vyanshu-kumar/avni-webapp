...
export const getNewEligibleEncounters = (encounterTypes, eligibleEncounters) => {
  ...
  const unplannedEncounters = get(eligibleEncounters, "eligibleEncounterTypeUUIDs", []).map(uuid => {
    const encounter = new Encounter();
    encounter.encounterType = encounterTypes.find(eT => eT.uuid === uuid);
    encounter.name = encounter.encounterType && encounter.encounterType.operationalEncounterTypeName;
    return encounter;
  });
